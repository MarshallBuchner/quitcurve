...
export async function sendWelcomeEmail({
  ...
  const remindersUrl = `${siteUrl}/reminders`;
...
<body style="margin:0;padding:0;background:#0a0f0d;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
        ...
        <table width="100%" style="max-width:480px;background:#121a17;border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:32px;">
          <tr>
            <td>
              ...
              <h1 style="margin:0 0 12px;font-size:24px;line-height:1.3;color:#f4f7f5;">Welcome, ${escapeHtml(firstName)}.</h1>
              <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#9aaba3;">
                Your quit plan is ready. Slip once — don't start over. Open QuitCurve anytime to log a craving, check in, and see progress.
              </p>
              <a href="${dashboardUrl}"
              ...
              <p style="margin:28px 0 0;font-size:12px;line-height:1.5;color:#6b7c74;">
                Want a daily nudge?
                <a href="${remindersUrl}" style="color:#5ee9b5;">Turn on check-in reminders</a>
              </p>
            </td>
          </tr>
        </table>
  ...
  const text = `Welcome, ${firstName}.\n\nYour QuitCurve plan is ready. Slip once — don't start over.\n\nOpen your plan: ${dashboardUrl}\nTurn on reminders: ${remindersUrl}`;
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [to],
      subject: "Welcome to QuitCurve — your plan is ready",
      html,
      text,
    }),
  });
